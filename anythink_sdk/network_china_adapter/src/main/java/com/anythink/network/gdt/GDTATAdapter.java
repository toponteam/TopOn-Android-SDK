...
public class GDTATAdapter extends CustomNativeAdapter {
        ...
        GDTATInitManager.getInstance().initSDK(context, serverExtras, new GDTATInitManager.OnInitCallback() {
            ...
            public void onError() {
                if (mCustomNativeListener != null) {
                    mCustomNativeListener.onNativeAdFailed(GDTATAdapter.this, ErrorCode.getErrorCode(ErrorCode.noADError, "", "GDT initSDK failed."));
                }
            }
        });
    }
    ...
    private void initUnifiedAd(final Context context) {
        mNativeUnifiedAd = new NativeUnifiedAD(context, mUnitId, new NativeADUnifiedListener() {
            @Override
    ...
    private void initNativeExpressAD(final Context context) {
        ...
        nativeExpressAD = new NativeExpressAD(context, new ADSize(mAdWidth, mAdHeight), mUnitId, _nativeExpressADListener); // Context must be Activity
    }
    ...
    private void initNativeVideoAD(final Context context) {
        ...
        mADManager = new NativeMediaAD(context, mUnitId, listener);
    }
