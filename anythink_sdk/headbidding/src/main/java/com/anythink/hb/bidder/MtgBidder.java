...
public class MtgBidder implements Bidder {
    ...
    public void bid(final BidRequestInfo bidRequestInfo, String adType, long timeOutMS,
        ...
        try {
            ...
            curUnitId = bidRequestInfo.getPlacementId();
            curPlacementId = bidRequestInfo.getUnitPlacementId();
            ...
            try {
                CustomInfoManager.getInstance().setCustomInfo(curUnitId, CustomInfoManager.TYPE_BID, curBidRequestInfo.getCustomInfo());
            } catch (Throwable e) {
            ...
            if (TextUtils.equals(ADType.BANNER, adType)) {// Banner
                if (TextUtils.equals(bannerSize, "smart")) {
                    ...
                    manager = new BidManager(new BannerBidRequestParams(curPlacementId, curBidRequestInfo.getPlacementId(), width, height));
                } else {
                    String[] size = bannerSize.split("x");
                    manager = new BidManager(new BannerBidRequestParams(curPlacementId, curBidRequestInfo.getPlacementId(), Integer.parseInt(size[0]), Integer.parseInt(size[1])));
                }
            } else {
    ...
    public Object getAdsRender() throws FailedToGetRenderException {
        ...
        switch ((String) adBidFormat) {
            case ADType.NATIVE: {
                Map<String, Object> properties = MtgBidNativeHandler.getNativeProperties(curUnitId);
                properties.put(MIntegralConstans.PROPERTIES_AD_NUM, 1);
                adsRender = new MtgBidNativeHandler(properties, mContext.getContext());
                break;
            }
            case ADType.INTERSTITIAL: {
                adsRender = new MTGBidInterstitialVideoHandler(mContext.getContext(), "", curUnitId);
                break;
            }
            case ADType.REWARDED_VIDEO: {
                adsRender = new MTGBidRewardVideoHandler(mContext.getContext(), "", curUnitId);
                break;
            }
            default:
                break;
