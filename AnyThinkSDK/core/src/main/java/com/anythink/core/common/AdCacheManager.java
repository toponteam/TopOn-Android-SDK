...
public class AdCacheManager {
    ...
    public AdCacheInfo checkCache(Context context, String placementId, boolean isAgent, boolean isShowCall, HashMap<Integer, ATMediationSetting> settings) {
        ...
        synchronized (AdCacheManager.this) {
            PlaceStrategy placeStrategy = getCacheStrategy(placementId);
            ...
            if (unitGroupInfos != null && unitGroupInfos.size() > 0) {
                ...
                for (PlaceStrategy.UnitGroupInfo unitGroupInfo : unitGroupInfos) {
                    ...
                    if (AdCapManager.getInstance(SDKContext.getInstance().getContext()).isUnitgroupOutOfCap(placementId, unitGroupInfo)) {
                    ...
                    if (unitgroupCacheInfo == null || unitgroupCacheInfo.getAdCacheInfo() == null) {
                        ...
                        if (baseAdapter != null) {
                            ...
                            if (isAdReady) {
                                ...
                                if (isAgent) {
                                    //Send isReady Agent
                                    AgentEventManager.isReadyEventAgentForATToApp(requestId, placementId, psid, sessionId, groupId, adTrackingInfo.getmRefresh()
                                            , true, -1, level, unitGroupInfo.unitId, unitGroupInfo.networkType, baseAdapter.getSDKVersion(), checkArray.toString(), currentRequestId, true, "");
                                }
                    ...
                    if (placeStrategy.getFormat() == 0) { // Native format
                    ...
                    if (isAdReady) {
                        if (adCacheInfo.getUpdateTime() + adCacheInfo.getCacheTime() > System.currentTimeMillis()) {
                            ...
                            if (isAgent) {
                                AgentEventManager.isReadyEventAgentForATToApp(requestId, placementId, psid, sessionId, groupId, adTrackingInfo.getmRefresh()
                                        , true, -1, level, unitGroupInfo.unitId, unitGroupInfo.networkType, baseAdapter.getSDKVersion(), checkArray.toString(), currentRequestId, adTrackingInfo.getRequestType() == AdTrackingInfo.READY_CHECK, "");
                            }
            ...
            if (myOfferUnitgroupInfoList != null && myOfferUnitgroupInfoList.size() > 0 && placeStrategy.getUseDefaultMyOffer() == 1) {
                ...
                if (defaultUnitGroupInfo != null) {
                    ...
                    try {
                        ...
                        if (isAdReady) {
                            ...
                            if (isAgent) {
                                AgentEventManager.isReadyEventAgentForATToApp(requestId, placementId, psid, sessionId, groupId, adTrackingInfo.getmRefresh()
                                        , true, -1, defaultUnitGroupInfo.level, defaultUnitGroupInfo.unitId, defaultUnitGroupInfo.networkType, baseAdapter.getSDKVersion(), checkArray.toString(), currentRequestId, true, defaultUnitGroupInfo.content);
                            }
            ...
            if (isAgent) {
                AgentEventManager.isReadyEventAgentForATToApp(requestId, placementId, psid, sessionId, groupId, 0,
                        false, AgentEventManager.NO_READY_ADSOURCE_REASON, -1, "", -1, "", checkArray.toString(), currentRequestId, false, "");
                if (isShowCall) { //Show Fail Agent
                    AgentEventManager.onAdShowFail(requestId, placementId, psid, sessionId, groupId, 0,
                            AgentEventManager.NO_READY_ADSOURCE_REASON, checkArray.toString(), currentRequestId);
                }
    ...
    private void initReadyCacheTrackingInfo(Context context, AnyThinkBaseAdapter adapter, String requestId, String placementId, PlaceStrategy placeStrategy, PlaceStrategy.UnitGroupInfo unitGroupInfo) {
        AdCapManager.CapInfo capInfo = AdCapManager.getInstance(context).getCapByPlaceIdAndUnitGroupId(placementId, unitGroupInfo.unitId);
        ...
        if (placeStrategy.getFormat() == 1) {
            adTrackingInfo.setmSourceType("1");
        } else {
            adTrackingInfo.setmSourceType("0");
        }

        adTrackingInfo.setmAdType(String.valueOf(placeStrategy.getFormat()));
        ...
        adTrackingInfo.setmSessionId(SDKContext.getInstance().getSessionId(placementId));
        ...
        adTrackingInfo.setmNetworkList(unitGroupInfo.networkType + "");
        adTrackingInfo.setmRequestNetworkNum(placeStrategy.getRequestUnitGroupNumber());
        adTrackingInfo.setmRefresh(0);
        adTrackingInfo.setmLevel(unitGroupInfo.level);
        ...
        adapter.setTrackingInfo(adTrackingInfo);

        adapter.setmUnitgroupInfo(unitGroupInfo);
        adapter.setRefresh(false);


        adTrackingInfo.setmNetworkVersion(adapter.getSDKVersion());
        adTrackingInfo.setNetworkName(adapter.getNetworkName());
            ...
            if (adTrackingInfo != null) {
                ...
                TaskManager.getInstance().run_proxy(new Runnable() {
                    ...
                    public void run() {
                        synchronized (AdCacheManager.this) {
                            //Save cap
                            AdCapManager.getInstance(context).saveOneCap(adTrackingInfo.getmPlacementId(), adTrackingInfo.getmUnitGroupUnitId());
                            //Recore time
     ...
     **/
    public synchronized void refreshCacheInfo(List<PlaceStrategy.UnitGroupInfo> unitGroupInfoList, String placementId, PlaceStrategy placementStrategy, AdTrackingInfo adTrackingInfo) {
        for (PlaceStrategy.UnitGroupInfo unitGroupInfo : unitGroupInfoList) {
            ...
            if (unitGroupCacheInfoMap != null) {
                ...
                if (adCacheInfo != null
                    ...
                    if (adCacheInfo.getOriginRequestId().equals(adTrackingInfo.getmRequestId())) {
                        continue;
                    }

                    if (adCacheInfo.isUpStatusAvaiable() && adCacheInfo.isNetworkAdReady()) {
                        /**Refresh Adaper's Tracking Info**/
                        TrackingInfoUtil.initPlacementUnitGroupTrackingInfo(adCacheInfo.getBaseAdapter(), adTrackingInfo, unitGroupInfo);
                        adTrackingInfo.setRequestType(4); //4:upstatus=1 and upstatus is available

                        //Send Agent
                        AgentEventManager.upStatusAvaibleCache(adTrackingInfo.getmRequestId()
                                , placementId
                                , SDKContext.getInstance().getPsid()
                                , SDKContext.getInstance().getSessionId(placementId)
                                , adTrackingInfo.getmGroupId(), adTrackingInfo.getmNetworkType(), adTrackingInfo.getmUnitGroupUnitId(), adTrackingInfo.getmLevel(), adCacheInfo.getOriginRequestId());

                        unitgroupCacheInfo.refreshCacheInfo(adTrackingInfo, placementStrategy, unitGroupInfo); //Refresh AdCache Info
                        continue;
                    }
                }
