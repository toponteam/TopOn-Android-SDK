...
public abstract class CommonCacheCountdownTimer extends CountDownTimer {
    ...
    public void onFinish() {
        ...
        if (mUnitgroupInfo.bidType == 1) {
            try {
                ...
                hbHandler.initHbInfo(context, mAdTrackingInfo.getmPlacementId(), Integer.parseInt(mAdTrackingInfo.getmAdType()), null, unitGroupInfos);
    ...
    private void prepareAdRequest(Context context) {
        ...
        mPlaceStrategy = AdCacheManager.getInstance().getCacheStrategy(mAdTrackingInfo.getmPlacementId());
    ...
    protected void onAdError(Context context, long startTime, AnyThinkBaseAdapter adapter, AdError adError) {
        ...
        if (!mHasGetResult) {
            ...
            AgentEventManager.onAdsourceLoadFail(adTrackingInfo.getmRequestId(), adTrackingInfo.getmPlacementId(), adTrackingInfo.getmPsid()
                    ...
                    , adTrackingInfo.getmLevel(), 0, adError, adTrackingInfo.getmBidType(), adTrackingInfo.getmBidPrice(), System.currentTimeMillis() - startTime);
